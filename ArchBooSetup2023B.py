...
def setup_prism():
    ...
    if "prism" in bpy.data.objects:
        ...
        bpy.data.objects["prism"].hide_set(False)
    ...
    if "heelLocator" in bpy.data.objects:
        ...
        bpy.ops.object.delete()
    if "orientationLocator" in bpy.data.objects:
        # Object exists, select it and delete it
        bpy.data.objects["orientationLocator"].select_set(True)
        bpy.ops.object.delete()

    #remove sub-collection if it exists:
    # Get the current collection
    curr_coll = bpy.context.collection

    # Check if the sub-collection exists
    sub_coll = curr_coll.children.get("hiddenLocators")

    # If it exists, delete its objects and unlink it from the current collection
    if sub_coll:
        # Select all objects in the sub-collection
        ...
        # Unlink the sub-collection from the current collection
        curr_coll.children.unlink(sub_coll)

        # Remove the sub-collection data-block
    ...
    ### make the upper locators not selectable
    #possibly already done?


    ###check if material exists, and if not, create and apply to prism.
    # Get the existing material by name
    prismat = bpy.data.materials.get ("PrismYellow")

    # If it does not exist, create a new one
    if prismat is None:
        prismat = bpy.data.materials.new (name="PrismYellow")
        prismat.diffuse_color = (0.5, 0.5, 0, 1)

    # Assign the material to the duplicate object
    obj.active_material = prismat


    ###Create and attach orientationLocator
    bpy.ops.object.empty_add(type='PLAIN_AXES', align='WORLD', location=(0, 0, 0), scale=(1, 1, 1))
    bpy.context.active_object.name='orientationLocator'

    ori = bpy.context.scene.objects.get("orientationLocator")

    # Add a copy location constraint to make the orientationLocator copy the location of mpa
    copy_loc_mpa = ori.constraints.new('COPY_LOCATION')
    copy_loc_mpa.target = aLoc

    # Add another copy location constraint to make the orientationLocator copy the location of mpb with influence 0.5
    copy_loc_mpb = ori.constraints.new('COPY_LOCATION')
    copy_loc_mpb.target = bLoc
    copy_loc_mpb.influence = 0.5

    #now point it at mph
    orient_h = ori.constraints.new('DAMPED_TRACK')
    orient_h.target = heelLoc
    orient_h.track_axis = 'TRACK_X'

    #now use mpa:
    orient_a = ori.constraints.new('LOCKED_TRACK')
    orient_a.target = aLoc
    orient_a.track_axis = 'TRACK_NEGATIVE_Y'
    orient_a.lock_axis = 'LOCK_X'

    ###move upper locators + orientationLocator to a new [hidden] collection to clean up outliner.

    # Get the current collection
    curr_coll = bpy.context.collection

    # Create a new sub-collection
    sub_coll = bpy.data.collections.new("hiddenLocators")

    # Link the sub-collection to the current collection
    curr_coll.children.link(sub_coll)

    # List of object names to move
    obj_names = ["aUp", "bUp", "heelUp", "orientationLocator"]

    # Loop through all objects in data
    for objs in bpy.data.objects:
        # Check if object name is in the list
        if objs.name in obj_names:
            # Loop through all collections the obj is linked to
            for coll in objs.users_collection:
                # Unlink the object
                coll.objects.unlink(objs)

            # Link the object to the sub-collection
            sub_coll.objects.link(objs)
